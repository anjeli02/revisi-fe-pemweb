...
import { getProducts, getTestimonialsApi } from "../services/skincareApi";
import { useAuthStore } from "../store/useAuthStore";
import skintific from "../assets/skintific.jpg";
...
export default function Beranda() {
  ...
  useEffect(() => {
    getTestimonialsApi().then(setTestimonials).catch(() => setTestimonials([]));
  }, []);
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 pt-16 pb-24 grid lg:grid-cols-2 gap-14 items-center relative">
          ...
          <div className="relative flex items-center justify-center">
            {/* Background blob with image */}
            <div className="w-80 h-80 bg-gradient-to-br from-brand-100 to-peach-100 rounded-[3rem] rotate-6 overflow-hidden">
              <img
                src={skintific}
                alt="Skintific Acne Serum"
      ...
      {/* ── TESTIMONIALS (dinamis dari data rating) ─────────────────── */}
      {testimonials.length > 0 && (
        <section className="bg-white border-y border-rose-100 py-20">
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {testimonials.map((t) => (
                <div key={t.id} className="bg-[#fffaf9] rounded-3xl border border-rose-50 p-6">
                  <StarRating count={Math.round(t.average)} />
                  <p className="text-stone-600 text-sm leading-relaxed mt-3 mb-5">"{t.review}"</p>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-stone-800 text-sm">{t.userName}</p>
                      <p className="text-xs text-stone-400">Pengguna GlowMatch</p>
                    </div>
                  </div>
                  <div className="mt-4 pt-4 border-t border-rose-100">
                    <p className="text-xs text-stone-400">Produk direview</p>
                    <p className="text-xs font-semibold text-brand-600">{t.productName}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-gradient-to-br from-brand-500 to-brand-700 rounded-[2.5rem] px-8 py-14 text-center relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <p className="text-white/75 mb-8 max-w-md mx-auto">
              {isAuthenticated
                ? "Temukan skincare yang paling cocok untuk kulitmu sekarang."
